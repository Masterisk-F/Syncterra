...
export const SyncProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const handleWebSocketMessage = useCallback((message: string) => {
        ...
        if (message.includes('Scan complete')) {
            ...
            setIsScanning(false);
            notifications.show({ title: 'スキャン完了', message: 'ファイルの読み込みが完了しました', color: 'green' });
        }
    ...
    const handleSync = useCallback(async (trackCount?: number) => {
        if (isSyncing || isScanning) {
            console.warn('Sync or Scan is already in progress. Ignoring handleSync call.');
            return;
        }
        ...
        try {
            ...
            addLog('同期開始失敗: ' + error);
            notifications.show({ title: 'エラー', message: '同期の開始に失敗しました', color: 'red' });
            setIsSyncing(false);
        }
    ...
    const handleScan = useCallback(async () => {
        if (isSyncing || isScanning) {
            console.warn('Sync or Scan is already in progress. Ignoring handleScan call.');
            return;
        }
