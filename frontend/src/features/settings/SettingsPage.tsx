...
const settingsArrayToApp = (settings: Setting[]): AppSettings => {
  ...
  return {
    ...
    targetExtensions:
      parseStringArray(settingsMap.get(SETTING_KEYS.targetExtensions)) ||
      DEFAULT_SETTINGS.targetExtensions,
    syncDestPath: settingsMap.get(SETTING_KEYS.syncDestPath) || '',
    syncMethod: (settingsMap.get(SETTING_KEYS.syncMethod) ||
      'adb') as SyncMethod,
    ftpHost: settingsMap.get(SETTING_KEYS.ftpHost),
    ftpPort: settingsMap.get(SETTING_KEYS.ftpPort)
      ? Number(settingsMap.get(SETTING_KEYS.ftpPort))
      : 21,
    ...
    rsyncPort: settingsMap.get(SETTING_KEYS.rsyncPort)
      ? Number(settingsMap.get(SETTING_KEYS.rsyncPort))
      : 22,
...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      // 各設定をバックエンドに送信
      await updateSetting(
        SETTING_KEYS.scanPaths,
        JSON.stringify(settings.scanPaths)
      );
      await updateSetting(
        SETTING_KEYS.excludeDirs,
        settings.excludeDirs.join(',')
      );
      await updateSetting(
        SETTING_KEYS.targetExtensions,
        settings.targetExtensions.join(',')
      );
      ...
      if (settings.ftpHost)
        await updateSetting(SETTING_KEYS.ftpHost, settings.ftpHost);
      if (settings.ftpPort)
        await updateSetting(SETTING_KEYS.ftpPort, String(settings.ftpPort));
      if (settings.ftpUser)
        await updateSetting(SETTING_KEYS.ftpUser, settings.ftpUser);
      if (settings.ftpPassword)
        await updateSetting(SETTING_KEYS.ftpPassword, settings.ftpPassword);

      if (settings.rsyncHost)
        await updateSetting(SETTING_KEYS.rsyncHost, settings.rsyncHost);
      if (settings.rsyncPort)
        await updateSetting(SETTING_KEYS.rsyncPort, String(settings.rsyncPort));
      if (settings.rsyncUser)
        await updateSetting(SETTING_KEYS.rsyncUser, settings.rsyncUser);
      if (settings.rsyncPassword)
        await updateSetting(SETTING_KEYS.rsyncPassword, settings.rsyncPassword);
      await updateSetting(
        SETTING_KEYS.rsyncUseKey,
        settings.rsyncUseKey ? '1' : '0'
      );
  ...
  return (
    <Stack gap="lg" maw={800}>
      ...
      <Paper p="md" withBorder radius="md">
        <Stack gap="md">
          ...
          <TagsInput
            ...
            onChange={(value) =>
              setSettings({ ...settings, excludeDirs: value })
            }
          />
          ...
          <TagsInput
            ...
            onChange={(value) =>
              setSettings({ ...settings, targetExtensions: value })
            }
          />
        </Stack>
      </Paper>
      ...
      <Paper p="md" withBorder radius="md">
        <Stack gap="md">
          ...
          <Select
            ...
            onChange={(value) =>
              setSettings({ ...settings, syncMethod: value as SyncMethod })
            }
          />
          ...
          <TextInput
            ...
            onChange={(e) =>
              setSettings({ ...settings, syncDestPath: e.target.value })
            }
            required
          />
          ...
          {settings.syncMethod === 'ftp' && (
            <>
              ...
              <TextInput
                ...
                onChange={(e) =>
                  setSettings({ ...settings, ftpHost: e.target.value })
                }
                required
              />
              <NumberInput
                ...
                onChange={(value) =>
                  setSettings({ ...settings, ftpPort: Number(value) })
                }
                min={1}
                max={65535}
              />
              <TextInput
                ...
                onChange={(e) =>
                  setSettings({ ...settings, ftpUser: e.target.value })
                }
                required
              />
              <PasswordInput
                ...
                onChange={(e) =>
                  setSettings({ ...settings, ftpPassword: e.target.value })
                }
                required
              />
            </>
          ...
          {settings.syncMethod === 'rsync' && (
            <>
              ...
              <TextInput
                ...
                onChange={(e) =>
                  setSettings({ ...settings, rsyncHost: e.target.value })
                }
                required
              />
              <NumberInput
                ...
                onChange={(value) =>
                  setSettings({ ...settings, rsyncPort: Number(value) })
                }
                min={1}
                max={65535}
              />
              <TextInput
                ...
                onChange={(e) =>
                  setSettings({ ...settings, rsyncUser: e.target.value })
                }
                required
              />
              ...
              <Switch
                ...
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    rsyncUseKey: e.currentTarget.checked,
                  })
                }
              />
              ...
              {settings.rsyncUseKey ? (
                <>
                  <Alert
                    ...
                    color="blue"
                  >
                    公開鍵をダウンロードして、同期先マシンの{' '}
                    <code>~/.ssh/authorized_keys</code> に追記してください。
                  </Alert>
                  <Button
                ...
                <PasswordInput
                  ...
                  onChange={(e) =>
                    setSettings({ ...settings, rsyncPassword: e.target.value })
                  }
                  required
                />
              )}
        ...
        <Stack gap="md">
          ...
          <Group>
            ...
            <Button
              ...
              leftSection={<IconTableMinus size={20} />}
            >
              トラックを選択して削除
            </Button>
          </Group>
      ...
      <Modal opened={opened} onClose={close} title="確認" centered>
        <Text size="sm">
          スキャンで見つからなくなったファイル（Missing状態）をデータベースから完全に削除しますか？
          <br />
          この操作は取り消せません。
        </Text>
        <Group justify="flex-end" mt="lg">
          ...
          <Button color="red" onClick={handleDeleteMissing}>
            削除
          </Button>
        </Group>
      </Modal>
      ...
      >
        <Text size="sm">
          選択した {gridApi?.getSelectedNodes().length}{' '}
          件のトラック情報を削除しますか？
          <br />
          （ファイル自体は削除されません）
        </Text>
        <Group justify="flex-end" mt="lg">
          ...
          <Button color="red" onClick={executeDeleteTracks}>
            削除
          </Button>
        </Group>
      </Modal>
