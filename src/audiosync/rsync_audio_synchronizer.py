    ...
    def synchronize(self, checksum=False):
        ...
        for audio in self.audio_sync_data.sheet_Albums:
            if getattr(audio, "sync", None) == "○":
                rel_path = audio.filepath_to_relative.replace(
        ...
        temp_dir = (
            tempfile.mkdtemp()
        )  # リモートのディレクトリ内のm3u8などのファイルを削除するため使用
        try:
            ...
            # sortとuniqをコマンドで実行
            subprocess.run(
                f"sort {include_path} | uniq > {include_path}.tmp && mv {include_path}.tmp {include_path}",
                shell=True,
                check=True,
            )
            ...
            for name, playlist in self.audio_sync_data.sheets_playlist.items():
                ...
                try:
                    ...
                    try:
                        ...
                        if proc.returncode != 0:
                            print(
                                f"rsync playlist failed: returncode {proc.returncode}"
                            )
                    except Exception as e:
