...
class AudioSyncData:
    ...
    class Audio:
        ...
        def __init__(self, filepath=None, relative_filepath=None):
            ...
            self.codec, 
            self.added_date, 
            self.update_date,) = [None for _ in range(15)]
            ...
            self.update_date = datetime.datetime.fromtimestamp(os.stat(filepath).st_mtime).isoformat(sep=" ",timespec="seconds")
            ...
            if os.path.splitext(filepath)[1] == ".mp3":
            ...
            elif os.path.splitext(filepath)[1] == ".mp4" or os.path.splitext(filepath)[1] == ".m4a":
                ...
                if self.track_num != "":
                    if self.track_num[1] == 0:
                        ...
                        self.track_num = str(min(self.track_num)) + "/" + str(max(self.track_num))
...
#                print(self.album_artist.encode("raw_unicode_escape").decode("shift_jis"))
    ...
    class __Sheet:
        ...
        def __init__(self, sheet, playlist=False):
            ...
            for row in range(min_row, self.__sheet.max_row + 1):
                if self.__sheet.cell(row=row,column=self.header["ファイルパス"]).value is None:
                    continue
                audio = AudioSyncData.Audio()
                audio.msg = None
                audio.sync = self.__sheet.cell(row=row,column=self.header["sync"]).value
                audio.title = self.__sheet.cell(row=row,column=self.header["タイトル"]).value
                audio.artist = self.__sheet.cell(row=row,column=self.header["アーティスト"]).value
                audio.album_artist = self.__sheet.cell(row=row,column=self.header["アルバムアーティスト"]).value
                audio.composer = self.__sheet.cell(row=row,column=self.header["作曲者"]).value
                audio.album = self.__sheet.cell(row=row,column=self.header["アルバム"]).value
                audio.track_num = self.__sheet.cell(row=row,column=self.header["#"]).value
                audio.length = self.__sheet.cell(row=row,column=self.header["長さ"]).value
                audio.filename = self.__sheet.cell(row=row,column=self.header["ファイル名"]).value
                audio.filepath_from = self.__sheet.cell(row=row,column=self.header["ファイルパス"]).value
                audio.filepath_to_relative = self.__sheet.cell(row=row,column=self.header["同期先相対ファイルパス"]).value
                audio.codec = self.__sheet.cell(row=row,column=self.header["コーデック"]).value
                audio.update_date = self.__sheet.cell(row=row,column=self.header["更新日時"]).value
                audio.added_date = self.__sheet.cell(row=row,column=self.header["追加日時"]).value
                #audio. = self.__sheet.cell(row=row,column=AudioSyncData.).value
                self.audio_dict[self.__sheet.cell(row=row,column=self.header["ファイルパス"]).value] = (row, audio)
        ...
        def delete_empty_row(self):
            for i in reversed(range(4 if self.__is_playlist else 2,self.__sheet.max_row)):
                if self.__sheet.cell(row=i, column=self.header["ファイルパス"]).value is None:
        ...
        def __setitem__(self, filepath, audio):
            ...
            self.__sheet.cell(row=row, column=self.header["タイトル"]).value = audio.title
            self.__sheet.cell(row=row, column=self.header["アーティスト"]).value = audio.artist
            self.__sheet.cell(row=row, column=self.header["アルバムアーティスト"]).value = audio.album_artist
            self.__sheet.cell(row=row, column=self.header["作曲者"]).value = audio.composer
            self.__sheet.cell(row=row, column=self.header["アルバム"]).value = audio.album
            ...
            self.__sheet.cell(row=row, column=self.header["ファイル名"]).value = audio.filename
            self.__sheet.cell(row=row, column=self.header["ファイルパス"]).value = audio.filepath_from
            self.__sheet.cell(row=row, column=self.header["同期先相対ファイルパス"]).value = audio.filepath_to_relative
            self.__sheet.cell(row=row, column=self.header["コーデック"]).value = audio.codec
            self.__sheet.cell(row=row, column=self.header["更新日時"]).value = audio.update_date
            self.__sheet.cell(row=row, column=self.header["追加日時"]).value = audio.added_date
        ...
        #戻り値 : 文字列。m3u8プレイリスト
            ...
            for row in range(min_row, self.__sheet.max_row + 1):
                try:
                    filepath = self.__sheet.cell(row=row, column=self.header["同期先相対ファイルパス"]).value[1:].replace(os.sep, sep)
                    title = self.__sheet.cell(row=row, column=self.header["タイトル"]).value
                    sync = self.__sheet.cell(row=row, column=self.header["sync"]).value
                    if filepath != "" and sync == "○":
                        rt += "#EXTINF:-1," + title + "\n"
                        rt += filepath + "\n\n"
                except TypeError:
                    logger.debug("Playlist \"" + str(self.__sheet.cell(row=1, column=3).value) + "\" : processing skip of row " + str(row))
            return rt
    ...
    def sheet_Not_in_Albums(self):
        ...
        if self.__sheet_Not_in_Albums is None:
            self.__sheet_Not_in_Albums = AudioSyncData.__Sheet(self.__workbook["Not in Albums"])
    ...
    def get_audio_file_list(self):
        ...
        for filepath, relative_filepath in self.get_audio_filepath_list():
            ...
            rt = AudioSyncData.Audio(filepath=filepath,relative_filepath=relative_filepath)
