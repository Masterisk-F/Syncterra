...
class AudioSynchronizer(metaclass=ABCMeta):
    def __init__(self, audio_sync_data, remote_os_sep):
        ...
        self.__remote_os_sep = (
            remote_os_sep  # 同期先における、ディレクトリ区切り文字(Winとlinux系で違う)
        )
    ...
    def synchronize(self):
        ...
        def make_list_remote(relative_path, root_path):
            logger.debug(
                'execute "ls" on remote. relative_path='
                + relative_path
                + ", root_path="
                + root_path
            )
            ...
            try:
                for filename, is_dir in self.ls_remote(relative_path):
                    if is_dir:
                        ...
                        make_list_remote(
                            relative_path + self.__remote_os_sep + filename, root_path
                        )
                    else:
                        logger.debug(
                            relative_path
                        ...
                        dict_to[
                            relative_path.replace(self.__remote_os_sep, os.sep)
                            + os.sep
                            + filename
                        ] = False
        ...
        for relative_filepath, chkflg in dict_to.items():
            if chkflg == False:
                if (
                    os.path.splitext(relative_filepath)[1]
                    in self.audio_sync_data.include_extention
                ):
                    logger.info(
                        "remove file on remote. path="
                        + relative_filepath.replace(os.sep, self.__remote_os_sep)
                    )
                    print(
                        "remove file on remote. path="
                        + relative_filepath.replace(os.sep, self.__remote_os_sep)
        ...
        for relative_filepath, (chkflg, audio) in dict_from.items():
            if chkflg == False and audio.sync == "○":
                ...
                print(
                    ...
                    + str(n)
                    + " Copy file. path="
                    + audio.filepath_to_relative.replace(os.sep, self.__remote_os_sep)
                )
                try:
                    # TODO : ディレクトリ存在確認とりあえずの実装。existsメソッドとか追加したほうが良いかも
                    self.ls_remote(
                        os.path.dirname(audio.filepath_to_relative).replace(
                            os.sep, self.__remote_os_sep
                        )
                    )
                except FileNotFoundError:
                    logger.info(
                        "make directory on remote. path="
                        + os.path.dirname(audio.filepath_to_relative).replace(
                            os.sep, self.__remote_os_sep
                        )
                    )
...
class AdbAudioSynchronizer(AudioSynchronizer):
    ...
    def cp(self, filepath_from, relative_path_to):
        ...
        command = (
            ...
            + filepath_from
            + '" "'
            + (self.audio_sync_data.dir_to_synchronize[0] + relative_path_to)
            + '"'
        )
    ...
    def rm_remote(self, relative_filepath_to):
        ...
        command = (
            'adb shell rm -f "'
            + self.adb_escape(
                self.audio_sync_data.dir_to_synchronize[0] + relative_filepath_to
            )
            + '"'
        )
        result = subprocess.run(
    ...
    def ls_remote(self, relative_dir=""):
        ...
        command = (
            'adb shell ls "'
            + self.adb_escape(self.audio_sync_data.dir_to_synchronize[0] + relative_dir)
            + '" -F1'
        )
        ...
        if result.stderr != "" and result.stderr[:-1].endswith(
            ...
            raise FileNotFoundError(
                ...
                self.audio_sync_data.dir_to_synchronize[0] + relative_dir,
            )
...
class FtpAudioSynchronizer(AudioSynchronizer):
    ...
    def ls_remote(self, relative_dir="/"):
        ...
        if not exists_dir(relative_dir):
            raise FileNotFoundError(
                errno.ENOENT, os.strerror(errno.ENOENT), relative_dir
            )
