...
@router.put("/batch")
async def batch_update_tracks(batch: BatchTrackUpdate, db: AsyncSession = Depends(get_db)):
...
@router.put("/{id}")
async def update_track(id: int, update: TrackUpdate, db: AsyncSession = Depends(get_db)):
