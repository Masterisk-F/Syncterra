...
def generate_ssh_key_pair(comment: str = ""):
    ...
    logger.info(
        f"SSH key pair generated at {SSH_PRIVATE_KEY_PATH} with comment '{comment}'"
    )
...
async def get_public_key(db: AsyncSession = Depends(get_db)):
    ...
    try:
        # 既存の鍵ファイルをチェック
        if not (
            os.path.exists(SSH_PRIVATE_KEY_PATH) and os.path.exists(SSH_PUBLIC_KEY_PATH)
        ):
            ...
            logger.info(
                f"SSH key pair not found, generating new one with comment: {comment}"
            )
            generate_ssh_key_pair(comment)

            # 鍵パスをデータベースに保存
            result = await db.execute(
                select(Setting).where(Setting.key == "rsync_key_path")
            )
            existing = result.scalars().first()
            if existing:
        ...
        raise HTTPException(
            status_code=500, detail=f"SSH key generation failed: {e.stderr}"
        )
    except Exception as e:
        logger.error(f"Public key retrieval error: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Public key retrieval error: {str(e)}"
        )
