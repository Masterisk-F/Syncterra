...
async def test_syncer_flow_adb(temp_db, create_settings, patch_db_session):
    ...
    # ルート: /local
    await create_track(temp_db, "/local/Artist/Album/song1.mp3", "Artist/Album/song1.mp3", sync=True, file_name="song1.mp3")
    # 除外
    await create_track(temp_db, "/local/Artist/Album/song2.mp3", "Artist/Album/song2.mp3", sync=False, file_name="song2.mp3")

    # 3. モック準備 & 実行
    # run_in_threadpool: 同期関数を非同期で呼ぶラッパーを無効化（即時実行）
    # subprocess.run: 実際のコマンド実行を阻止して検証
    with patch("backend.core.syncer.run_in_threadpool", side_effect=lambda f, *args: f(*args)), \
        ...
        # song1.mp3 は転送されるべき
        song1_pushed = any("/local/Artist/Album/song1.mp3" in str(c) for c in push_calls)
        assert song1_pushed, "song1.mp3 should be pushed via ADB"
        
        # song2.mp3 は転送されるべきではない
        song2_pushed = any("/local/Artist/Album/song2.mp3" in str(c) for c in push_calls)
...
async def test_syncer_flow_rsync(temp_db, create_settings, patch_db_session):
    ...
    # 2. データ準備
    await create_track(temp_db, "/local/music/Artist/song1.mp3", "Artist/song1.mp3", sync=True)

    # 3. モック準備 & 実行
    with patch("backend.core.syncer.run_in_threadpool", side_effect=lambda f, *args: f(*args)), \
         patch("subprocess.Popen") as mock_popen, \
         patch("subprocess.run"): # For mkdir/cp commands if any
