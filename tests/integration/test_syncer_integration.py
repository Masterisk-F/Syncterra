...
async def create_track(db, file_path, relative_path, sync=True, file_name="file.mp3"):
    t = Track(
        file_path=file_path, relative_path=relative_path, file_name=file_name, sync=sync
    )
    db.add(t)
    await db.commit()
    return t
...
async def test_syncer_adb_integration(temp_db):
    ...
    await create_track(
        db, "/local/song2.mp3", "/Artist/Album/song2.mp3", sync=False
    )  # Should skip
    ...
    with (
        patch("backend.core.syncer.AsyncSessionLocal", return_value=mock_session_cls),
        patch(
        ...
        expected_cmd = [
            ...
            "push",
            "/local/song1.mp3",
            "/sdcard/Music/Artist/Album/song1.mp3",
        ]
