...
from backend.core.syncer import (
    SyncService,
    AdbSynchronizer,
    RsyncSynchronizer,
    FtpSynchronizer,
...
def test_make_m3u8():
    ...
    t2 = Track(
        file_name="song2.mp3", title=None, relative_path="/Album/song2.mp3"
    )  # No title
...
async def test_sync_service_run_adb(db_session):
    ...
    with (
        patch(
            "backend.core.syncer.run_in_threadpool", side_effect=lambda f: f()
        ) as mock_thread,
...
def test_rsync_synchronizer(mock_run):
    ...
    with (
        patch("tempfile.mkstemp", return_value=(1, "/tmp/tmpxxx")),
        patch("os.close"),
        patch("os.remove"),
        ...
        playlist_calls = [
            c for c in mock_run.call_args_list if len(c[0]) > 0 and "rsync" in c[0][0]
        ]
        assert len(playlist_calls) >= 1, (
            "プレイリストファイルがrsyncで転送されていません"
        )
        # 最後の呼び出しにTestPlaylist.m3uが含まれているか確認
        last_call_cmd = playlist_calls[-1][0][0]
        assert "TestPlaylist.m3u" in " ".join(last_call_cmd), (
            "プレイリスト名がコマンドに含まれていません"
        )
