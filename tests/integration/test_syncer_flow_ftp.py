...
class QuietFTPHandler(FTPHandler):
    ...
    def logerror(self, msg, *args, **kwargs): pass
...
@pytest.mark.asyncio
async def test_syncer_flow_ftp(temp_db, temp_fs, create_settings, patch_db_session, ftp_server):
    """
    [Syncer] FTP同期フロー
    ...
    assert os.path.exists(dest_path_song1), "song1.mp3 がFTPサーバーに転送されていること"
    
    # Sync=False の確認
    dest_path_song2 = os.path.join(server_root, "Artist1", "Album1", "song2.mp3")
    assert not os.path.exists(dest_path_song2), "song2.mp3 (sync=False) は転送されていないこと"
