...
async def seed_tracks():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        async with AsyncSessionLocal() as session:
             ...
             t1 = Track(file_path="/music/t1.mp3", relative_path="t1.mp3", file_name="t1", title="Title1", sync=False)
             t2 = Track(file_path="/music/t2.mp3", relative_path="t2.mp3", file_name="t2", title="Title2", sync=True)
             session.add(t1)
             session.add(t2)
